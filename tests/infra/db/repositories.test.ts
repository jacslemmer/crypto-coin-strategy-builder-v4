...
import type { NewPair, NewChart, NewChartAnalysis, NewJob, NewVersion, NewImage } from '../../../src/infra/db/schema.js';
...
describe('Database Repositories', () => {
  ...
  describe('Pairs Repository', () => {
    ...
    it('should find top pairs by market cap', async () => {
      ...
      expect(topPairs[0]?.symbol).toBe('BTCUSDT'); // Highest market cap
      expect(topPairs[1]?.symbol).toBe('ETHUSDT'); // Lower market cap
    });
  });
  ...
  describe('Chart Analyses Repository', () => {
    ...
    it('should create a new chart analysis', async () => {
      const analysisData: NewChartAnalysis = {
        id: 'analysis-1',
        chartId,
        pairId,
        aiName: 'Gemini',
    ...
    it('should find top ranked analyses', async () => {
      const analyses: NewChartAnalysis[] = [
        {
          id: 'analysis-1',
          chartId,
          pairId,
  ...
  describe('Repository Integration', () => {
    it('should maintain referential integrity between tables', async () => {
      ...
      // Create a chart linked to the pair
      const chart = await repo.charts.create({
        id: 'chart-1',
        pairId: pair.id,
      ...
      // Create an analysis linked to the chart
