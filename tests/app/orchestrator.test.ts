...
import type { Capturer, CoinSource, FetchJobParams, IdGenerator, PairResolver, PersistenceAdapter, StorageAdapter, VersionRecord, ImageRecord, ProgressLogger } from '../../src/ports.js';
...
describe('runFetchJob', () => {
  it('processes only USDT pairs and stores full + anon images', async () => {
    const params: FetchJobParams = { limit: 5, source: 'both', includeAnonymized: true };

    const source: CoinSource = {
      async listTopSymbols() {
        return ['BTC', 'ETH', 'ABC'];
      },
    };
    const resolver: PairResolver = {
      async resolvePreferredUsdtPair(symbol) {
        if (symbol === 'ABC') return null; // simulate no USDT pair
    ...
    const ids: IdGenerator = { generateId: () => 'id' };
    const now = () => new Date('2020-01-01T00:00:00.000Z');
    const logs: string[] = [];
    const logger: ProgressLogger = { log: async (m) => { logs.push(m); } };
